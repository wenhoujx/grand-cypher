...
class TestModeling:
    def test_simple(self):
        conn = load_from_schema(_load_yaml("testing/test_schemas/one_node_one_table.yml"))
    ...
    def test_one_node_two_tables(self): 
        conn = load_from_schema(_load_yaml("testing/test_schemas/one_node_two_tables.yml"))
        res = conn.execute("match (p:Person)  return p").get_as_arrow(chunk_size=200)
        assert res.num_rows == 10, "10 rows expected"
        res = conn.execute("match (p:Person {name: 'Mary Anderson'}) return p.gender as gender").get_as_df()
